...
class TypeScriptParser(BaseParser):
    ...
    def _walk_top_level(
        ...
        for child in node.children:
            ...
            if ntype == "import_statement":
                ...
                self._handle_class(child, src, file_id, symbols, import_edges, type_edges, raw_calls)
                ...
                self._handle_namespace(child, src, file_id, symbols, import_edges, type_edges, raw_calls)
            elif ntype == "ambient_declaration":
                self._handle_ambient(child, src, file_id, symbols, import_edges, type_edges, raw_calls)
    ...
    def _handle_export(
        ...
        for child in node.children:
            ...
            if ntype in ("class_declaration", "abstract_class_declaration"):
                ...
                self._handle_var_decl(child, src, file_id, symbols, exported=True, raw_calls=raw_calls)
            elif ntype == "interface_declaration":
                ...
                self._handle_namespace(child, src, file_id, symbols, import_edges, type_edges,
                                       raw_calls, exported=True)
            elif ntype == "ambient_declaration":
    ...
    def _handle_class(
        ...
        name_node = (
            self._get_child_by_type(node, "type_identifier")
        ...
        if body_node:
            ...
            for child in body_node.children:
                if child.type == "method_definition":
                    ...
                    self._handle_abstract_method(child, src, file_id, symbols, class_local_idx, name)
                ...
                elif child.type == "public_field_definition" and field_count < self.MAX_CLASS_FIELDS:
    ...
    def _handle_method(
        ...
        method_decs = [self._extract_decorator_text(c, src)
                       for c in node.children if c.type == "decorator"]
    ...
    def _handle_class_field(
        ...
        """
        name_node = (
            self._get_child_by_type(node, "property_identifier")
            or self._get_child_by_type(node, "identifier")
        )
        if not name_node:
            return
        ...
        field_decs = [self._extract_decorator_text(c, src)
                      for c in node.children if c.type == "decorator"]
    ...
    def _handle_interface(
        ...
        if body_node:
            self._extract_interface_members(body_node, src, file_id, symbols,
                                             interface_local_idx, name, exported)
    ...
    def _extract_interface_members(
        ...
        for child in body_node.children:
            ...
            if child.type in ("method_signature", "property_signature"):
                ...
                kind = SymbolKind.FUNCTION if child.type == "method_signature" else SymbolKind.VARIABLE
                symbols.append(Symbol(
                    file_id=file_id,
                    name=member_name,
    ...
    def _handle_namespace(
        ...
        name_node = (
            self._get_child_by_type(node, "identifier")
            or self._get_child_by_type(node, "string")
        )
        if not name_node:
            return
        ...
        body_node = self._get_child_by_type(node, "statement_block")
        if body_node:
            self._walk_top_level(body_node, src, file_id, symbols, import_edges, type_edges, raw_calls)
    ...
    def _handle_ambient(
        ...
        for child in node.children:
            ntype = child.type
            if ntype in ("class_declaration", "abstract_class_declaration"):
                self._handle_class(child, src, file_id, symbols, import_edges, type_edges,
                                   raw_calls, exported=True)
            elif ntype in ("function_declaration", "generator_function_declaration"):
