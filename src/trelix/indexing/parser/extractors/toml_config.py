...
class TomlParser(BaseParser):
    ...
    def parse(self, source: str, file_id: int) -> ParseResult:
        ...
        for child in tree.root_node.children:
            ...
            if child.type == "pair":
                ...
                self._handle_pair(child, "", file_id, symbols, import_edges,
                                  src_bytes, depth=0, section_name="")
    ...
    def _handle_table(
        ...
        for child in table_node.children:
            if child.type == "pair" and len(symbols) < self.MAX_SYMBOLS:
                self._handle_pair(child, qualified_name, file_id, symbols, import_edges,
                                  src_bytes, depth=1, section_name=section_leaf)
        ...
        if val_node.type == "inline_table":
            ...
            if depth < self.MAX_DEPTH:
                for child in val_node.children:
                    if child.type == "pair" and len(symbols) < self.MAX_SYMBOLS:
                        self._handle_pair(child, path, file_id, symbols, import_edges,
                                          src_bytes, depth + 1, section_name=key)

        elif val_node.type == "array":
            item_count = sum(1 for c in val_node.children
    ...
    def _emit_dep_edges(
        ...
        for child in table_node.children:
            if child.type != "pair":
                continue
            key_node = next(
                (c for c in child.children if c.type in _KEY_TYPES), None
            )
            if key_node is None:
                continue
    ...
    def _find_pair_value(self, table_node: Any, key: str, src_bytes: bytes) -> Optional[str]:
    ...
    def _get_preceding_comment(self, node: Any, src_bytes: bytes) -> Optional[str]:
        ...
        if prev is not None and prev.type == "comment":
            if prev.end_point[0] + 1 >= node.start_point[0]:
                raw = src_bytes[prev.start_byte:prev.end_byte].decode(
                    "utf-8", errors="replace"
                )
