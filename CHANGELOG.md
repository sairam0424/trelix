...
### Overview
Six backlog phases plus three follow-up fixes: connector-fetched artifacts now auto-link into `generic_edges` on sync, opt-in Personalized PageRank for cross-source ranking, a unified retry/backoff contract shared by every LLM backend/embedder/connector/reranker/PR client, structured JSON logging with trace correlation, a new Xray Cloud connector, and a new Linear connector — plus two real bugs found by live-testing the Jira connector against a production Jira Cloud site (ADF descriptions silently dropped to empty bodies; bad credentials silently reported as success) and a `.env`-leakage test-isolation fix. All new surface is additive/opt-in; no breaking changes.
