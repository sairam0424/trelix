...
**v3.0.0 — Thinking, Budgets, Audit & SSO:** Six feature areas, all additive and off by default. Anthropic **extended thinking** as a per-call-site request parameter (`TRELIX_LLM_THINKING_ENABLED`), opted into only by `RetrievalSynthesizer` — never by the index-time LLM calls. A **model-aware context budget** that derives the packer's ceiling from the synthesis model's real context window (`TRELIX_RETRIEVAL_CONTEXT_TOKEN_BUDGET=null`) instead of the fixed `12000`. **SeleCom query-conditioned context compression** (`TRELIX_RETRIEVAL_COMPRESSION`) that compresses only the candidates that would otherwise be dropped, so the result set is a superset of the uncompressed one and recall cannot regress. A **hash-chained, append-only audit trail** in its own `audit.db` (`TRELIX_AUDIT_ENABLED`) with `trelix audit list` / `verify` / `export`. **OIDC SSO** (`pip install 'trelix[sso]'`) keyed on `(sub, iss)` with an asymmetric-only algorithm allowlist that makes `alg: none` and HS256 confusion unreachable. And an **actionable VS Code extension** — code lenses, a hover provider, and an `@trelix` chat participant. Plus opt-in FTS5 declaration-boost BM25 ranking. See [CHANGELOG.md](CHANGELOG.md) for the six classes of live defect fixed along the way.
...
pip install trelix            # + OpenAI planner & synthesis
pip install "trelix[all]"     # + local embedder, voyage, cohere rerank, sparse, qdrant, watch, serve, sso, otel, knowledge-graph
```

`[all]` is not every extra. It resolves to
`local,rerank,voyage,sparse,qdrant,watch,serve,sso,otel,knowledge-graph` — ten of the 23
...
| Variable | Default | Description |
|---|---|---|
| `TRELIX_TELEMETRY_ENABLED` | `false` | Record per-query latency, intent and result count to the `query_telemetry` table in `.trelix/index.db` |
...
# Full eval (trelix-self, whole golden set — nDCG@10 + Recall@10 + MRR)
make eval-full

# CoIR eval harness (v2.1.0) — run against your own golden set
# golden.jsonl format: {"query": "...", "relevant_files": ["path/to/file.py"]}
trelix eval ./my-repo --golden eval/golden.jsonl
```
