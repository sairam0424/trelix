...
MCP server for [trelix](https://github.com/sairam0424/trelix) v2.11.1 — semantic code search with streaming /ask endpoint, watch bridge notifications, and REST API integration for Claude Code, Cursor, Windsurf, and Continue.dev.
...
```bash
pip install trelix-mcp==2.11.1
```

To use Bedrock embeddings or synthesis (no extra API key beyond AWS credentials):

```bash
pip install "trelix-mcp==2.11.1" "trelix[bedrock]"
```

Other optional LLM provider extras:

```bash
pip install "trelix-mcp==2.11.1" "trelix[anthropic]"   # Anthropic Claude direct
pip install "trelix-mcp==2.11.1" "trelix[vertex]"       # Google Vertex AI / Gemini
pip install "trelix-mcp==2.11.1" "trelix[litellm]"      # 100+ providers via LiteLLM
pip install "trelix-mcp==2.11.1" "trelix[llm-all]"      # all LLM providers
```
...
```bash
pip install 'trelix-mcp==2.11.1' 'trelix[knowledge-graph]'
```
