...
MCP server for [trelix](https://github.com/sairam0424/trelix) v2.10.0 — semantic code search with streaming /ask endpoint, watch bridge notifications, and REST API integration for Claude Code, Cursor, Windsurf, and Continue.dev.
...
```bash
pip install trelix-mcp==2.10.0
```

To use Bedrock embeddings or synthesis (no extra API key beyond AWS credentials):

```bash
pip install "trelix-mcp==2.10.0" "trelix[bedrock]"
```

Other optional LLM provider extras:

```bash
pip install "trelix-mcp==2.10.0" "trelix[anthropic]"   # Anthropic Claude direct
pip install "trelix-mcp==2.10.0" "trelix[vertex]"       # Google Vertex AI / Gemini
pip install "trelix-mcp==2.10.0" "trelix[litellm]"      # 100+ providers via LiteLLM
pip install "trelix-mcp==2.10.0" "trelix[llm-all]"      # all LLM providers
```
...
```bash
pip install 'trelix-mcp==2.10.0' 'trelix[knowledge-graph]'
```
