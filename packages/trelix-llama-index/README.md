...
`flare_max_retries` replaces `flare_max_iterations` in `RetrievalConfig`. Only the new env var `TRELIX_RETRIEVAL_FLARE_MAX_RETRIES` is accepted — the old `TRELIX_RETRIEVAL_FLARE_MAX_ITER` was removed in v3.3.0 and now has no effect.
