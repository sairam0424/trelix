...
> **Historical record — command was renamed after this plan.** This document is a snapshot
> of the 2026-06-28 design. The symbol-inspection command described below as
> `trelix graph <repo> <symbol> --direction ...` shipped as **`trelix call-graph`**. The
> name `trelix graph` was later reused for the knowledge-graph command, which takes
> `--visualize`, `--output`, `--concepts`, and `--json` and has no `--direction` flag. Do
> not copy the `trelix graph ... --direction` commands below into a shell — use
> `trelix call-graph <repo> <symbol> --direction callers|callees|importers|all`. See
> [CLI_REFERENCE.md](../../CLI_REFERENCE.md) for the current surface.
