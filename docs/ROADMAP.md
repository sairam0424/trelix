...
- [x] **Remove deprecated** — the `TRELIX_RETRIEVAL_FLARE_MAX_ITER` **env-var alias**
      ...
      v3.3.0 actually removes, is only the legacy env name.
      ...
      **This slipped past v3.0.0**, which shipped 2026-08-13 with the alias still
      live; removal is only permitted in a release explicitly flagged for breaking
      changes — previously that meant a MAJOR bump, now it's a deliberately
...
- **CodeBERT fine-tuning** — domain-adapted embedding model trained on trelix's own telemetry data. `/deep-research` (2026-09) found no proven case for this over a strong off-the-shelf embedder — one study had `text-embedding-3-large` beat every fine-tuned model on one of three benchmarks — so before investing in a fine-tune, get a real baseline to beat. `query_telemetry` (see [OBSERVABILITY.md](OBSERVABILITY.md)) only records aggregate stats (elapsed_ms, result_count, an optional thumbs_up) — no actual chunk/file relevance labels — so it cannot directly seed a held-out query→correct-file set; `eval/golden.jsonl` (54 hand-curated, real trelix-codebase questions) is the closest existing substitute, and `trelix eval`/`EvalHarness` (`src/trelix/eval/harness.py`) already IS the recall@k harness this item used to call for building — no new harness was needed. Measured baselines (repo-relative rel_path matching confirmed correct against the real 604-file index; both plan-cached, and each reproduced byte-for-byte on a second run):
  - `local` embedder, no LLM credentials (fully offline, zero cost): nDCG@10 0.0669, Recall@10 0.0648, MRR 0.0741.
  - `azure` embedder + `azure` query planner (production-realistic — matches the GitHub App's now-fixed config): nDCG@10 0.0666, Recall@10 0.0833, MRR 0.0628.

  Both are surprisingly low, and it is NOT a harness bug: spot-checked one query directly against `Retriever.retrieve()` and confirmed a genuine semantic-retrieval weakness, not a broken measurement — `docs/CONFIGURATION.md` (one of two expected files) was correctly found at rank 1, but `src/trelix/indexing/walker.py` (the actual implementation) never appeared in the top 30, crowded out by config/dependabot/semgrep files that superficially mention "ignore" patterns without being the real answer. `QueryPlanner` (`retrieval/planner/agent.py`) derived its OWN LLM provider from `EmbedderConfig.provider`, not `TRELIX_LLM_PROVIDER` — there was no way to combine the free `local` embedder with a working LLM-backed query planner; a `local`-embedder run always fell back to `default_plan()` unless a real `OPENAI_API_KEY` happened to also be set. **Fixed 2026-09-22**: `QueryPlanner`/`AdaptiveRouter` gained an `llm_config` parameter that, when supplied, is used directly regardless of the embedder's own provider (mirroring `Synthesizer`'s pre-existing, identical pattern); `Retriever` now passes `config.llm` through. The measurements below predate the fix and are unaffected by it — both runs already had a real, matching-provider LLM config (`local`+no-LLM-credentials and `azure`+`azure` respectively), neither of which this coupling bug touched. n=54 on trelix's own single repo, and the queries are deliberately hard/specific (not "typical" easy lookups) — these numbers are a real, reproducible floor to beat, not a claim about retrieval quality in general. Fine-tuning (or any other technique) is only worth pursuing if it demonstrably beats these on the SAME golden set.
- **Semantic diff** — diff-aware retrieval (weight recently-changed symbols higher). Phase 2 Plan B built the supporting code (`DiffEmbedder`, `src/trelix/review/diff_embedder.py`) and the `diff_chunks` schema (`db.py`), both tested, but neither was ever called from `trelix review`'s actual retrieval path — "shipped" was inaccurate. Removed 2026-09-24 as dead code rather than integrated; revisit from git history if this becomes a real priority.
...
- **Multi-modal** — index diagrams, comments referencing architecture docs. `.drawio` diagrams shipped as a scoped pilot: `trelix connector sync <repo> diagram` captions local `.drawio` files via the existing text-only LLM client (XML source is fully describable as text — no new provider code needed) and links them as Artifacts via `ArtifactLinker`, same as a Jira ticket. Raster images (`.png`/`.jpg`) shipped the same pilot pattern: `ChatMessage` gained an additive `images` field and the Anthropic backend builds real Messages API image content blocks (`src/trelix/llm/client.py`, `src/trelix/llm/providers/anthropic_backend.py` — the other 4 backends raise `NotImplementedError` rather than silently mishandling images); `trelix connector sync <repo> image` (`src/trelix/indexing/connectors/image.py`) discovers `.png`/`.jpg`/`.jpeg` files via the same nested-`.gitignore` matching `FileWalker` uses (not a hand-maintained skip-list), downscales oversized images via Pillow before captioning, and falls back to a mechanical filename+dimensions+size description on any captioning failure — same never-worse-without-a-trace contract as the `.drawio` pilot. A live spike against 10 real architecture/UML/ER diagrams (`tests/fixtures/sadu_sample/`, sourced from the SADU benchmark, Zenodo DOI 10.5281/zenodo.19339991) found 2.8–4.0s caption latency and, for the 3 ER diagrams with ground-truth QA data, exact entity/relation-count matches despite the prompt never asking for counts — real evidence the caption-then-embed mechanism produces structurally accurate descriptions, not just plausible-sounding text. Known limitation carried over from the `.drawio` pilot: Artifacts (diagrams, images, tickets) link into the code graph via `generic_edges` but do not themselves surface as direct search results today — they appear only when a linked code symbol is retrieved. Not yet resolved: CPU latency for a local/offline vision model, and whether a lightweight ColPali-style reranker would close any real recall gap (no evidence of one found in the spike).
