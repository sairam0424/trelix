...
### Current version scope (v3.3.6)

| In v3.3.6 | Planned (post-3.1) |
|-----------|-----------------|
...
| Multi-repo watching (watchfiles.awatch) | |
| Knowledge graph (Louvain, Pyvis, BFS leg) | |
| FLARE confidence-gated re-retrieval | |
| Multi-granularity / sub-chunk statement-level search | |
| GroUSE-style LLM-as-judge synthesis eval harness | |
...
**LLM synthesis (`trelix ask`) needs an API key.** `trelix ask` — which synthesizes a natural-language answer from retrieved context — calls an LLM. You need at least one provider configured (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, AWS Bedrock credentials, etc.).
