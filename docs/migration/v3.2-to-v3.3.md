# Migrating from trelix v3.2.x to v3.3.0

**Scope:** v3.2.5 → v3.3.0. v3.3.0 is a deliberately elevated MINOR release — trelix
reserves breaking changes for a release like this rather than for a MAJOR bump (see
[BACKWARDS_COMPATIBILITY.md](../BACKWARDS_COMPATIBILITY.md)); this guide is a living
document, updated as each v3.3.0 change lands rather than written once at the end. Check
`docs/ROADMAP.md` for what's still in progress.
...
# After (v3.3.0 — the only name that binds)
...
The rest of v3.3.0's scope (LLM provider abstraction changes, MCP protocol updates) is
