...
| `TRELIX_RETRIEVAL_FLARE_MAX_RETRIES` | `1` | Max FLARE retries (range: 1–3). Replaces the old `TRELIX_RETRIEVAL_FLARE_MAX_ITER`, which was **removed in v4.0.0** — setting it now has no effect. Values > 3 raise `ValidationError` at startup. |
...
*End of CLI Reference — trelix v3.2.3*
