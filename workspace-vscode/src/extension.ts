...
import { TrelixCodeLensProvider } from "./code-lens-provider";
import { registerTrelixChatParticipant } from "./chat-participant";
...
type ResultItem = vscode.QuickPickItem & {
    itemKind: "result";
    result: SearchResult;
};
...
export async function activate(
    ...
    const codeLensProvider = new TrelixCodeLensProvider(
        ensureConnected,
        getRepoPath,
    );
    context.subscriptions.push(
        ...
        codeLensProvider,
        // Re-query lenses when the enable/disable setting flips.
        vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration("trelix.codeLens.enabled")) {
                codeLensProvider.refresh();
            }
        }),
    );

    // Chat participant (@trelix). Returns undefined on VS Code 1.90–1.94 where
    // the chat API is absent; the no-op fallback keeps activate() clean there.
    context.subscriptions.push(
        registerTrelixChatParticipant(context, {
    ...
    async function openResult(
        ...
        const absolutePath = vscode.Uri.file(
            path.isAbsolute(result.file)
                ? result.file
                : path.join(repoPath, result.file),
        );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("trelix.search", async () => {
            ...
            qp.placeholder =
                "Search your codebase with trelix — e.g. JWT token validation middleware";
            ...
            const controller = new SearchController({
                ...
                pageSize: PAGE_SIZE,
                search: (query, k, cursor) =>
                    c.search(query, repoPath, k, cursor),
                ...
                onError: (err) => {
                    vscode.window.showErrorMessage(
                        `trelix search failed: ${err instanceof Error ? err.message : String(err)}`,
                    );
                },
                onBusyChange: (busy) => {
                    qp.busy = busy;
            ...
            await vscode.window.withProgress(
                {
                    ...
                    title: "trelix: thinking…",
                },
                async () => {
                    const answer = (await c.ask(query, repoPath)).answer;
        ...
        vscode.commands.registerCommand(
            ...
            async (symbolName?: string) => {
                ...
                const page = await vscode.window.withProgress(
                    {
                        location: vscode.ProgressLocation.Notification,
                        title: `trelix: finding code similar to ${symbolName}…`,
                    },
                    () => c.search(symbolName, repoPath, PAGE_SIZE, 0),
                );
                if (page.results.length === 0) {
                    vscode.window.showInformationMessage(
                        `trelix: no similar code found for ${symbolName}.`,
                    );
                    return;
                }
                const picked = await vscode.window.showQuickPick(
                    page.results.map(toResultItem),
